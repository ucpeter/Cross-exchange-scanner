...
def run_scan():
    ...
    try:
        ...
        if results:
            ...
            def cpr(p):
                return (
                    f'<span class="good mono">{p:.4f}%</span>'
                    if p >= 0
                    else f'<span class="bad mono">{p:.4f}%</span>'
                )
            ...
            html = (
                '<div class="table-wrap"><table class="arb-table"><thead><tr>'
            ...
            st.info(
                "No opportunities matched your profit/volume/chain filters right now."
            )
    except Exception as e:
        st.error(f"Error: {e}")
